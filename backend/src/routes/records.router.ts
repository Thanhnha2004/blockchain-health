...
router.post("/", upload.single("file"), async (req: Request, res: Response) => {
  ...
  const tx = await getRecordRegistry().addRecord(
    patientDID,
    ipfsCID,
    dataHash,
    recordType,
...
router.get("/:patientDID", async (req: Request, res: Response) => {
  ...
  const didContract = getRecordRegistry(); // cần thêm DIDRegistry service
  const isOwner =
    (await getDIDRegistry().getDIDByAddress(callerAddress)) === patientDID;
  const canAccess =
    isOwner || (await getAccessControl().hasAccess(patientDID, callerAddress));
  ...
  if (aesKey) {
    // Lấy đúng record theo CID nếu có query param
    const { cid } = req.query as { cid?: string };

    const targetRecords = cid
      ? records.filter((r: any) => r.ipfsCID === cid)
      : records;

    const decryptedRecords = await Promise.all(
      targetRecords.map(async (record: any) => {
        try {
